...
def loadWorkday(engine):
	...
	#formerworkerws = '''<env:Envelope
    #xmlns:env="http://schemas.xmlsoap.org/soap/envelope/"
    #xmlns:xsd="http://www.w3.org/2001/XMLSchema">'''
    #formerworkerws += '''<env:Body>'''
	formerworkerws = '''<wd:Import_Former_Worker_Request xmlns:wd="urn:com.workday/bsvc" wd:version="v31.2">'''
	...
	for x in range (0,len(formerworker.index)):
		...
		if job.iloc[x]['former_worker_type'] != '':
			formerworkerws +='''<wd:Worker_Type_Reference> <wd:ID wd:type="Worker_Type_ID">{0}</wd:ID></wd:Worker_Type_Reference>'''.format(job.iloc[x]['former_worker_type'])
		...
		if (formerworker.iloc[x]['legal_local_first_name']+formerworker.iloc[x]['legal_local_first_name2']+formerworker.iloc[x]['legal_local_middle_name']+formerworker.iloc[x]['legal_local_last_name']+formerworker.iloc[x]['legal_local_last_name2']+formerworker.iloc[x]['legal_local_secondary_name'] != ''):
			...
			if (formerworker.iloc[x]['legal_local_middle_name'] != ''):
				formerworkerws += '''<wd:Middle_Name>{0}</wd:Middle_Name>'''.format(formerworker.iloc[x]['legal_local_middle_name'])
		...
		formerworkerws += parsePersonal(personal[personal['former_worker_id'] == formerworkerID])

		#Start contact data section of webservice.
		formerworkerws += '''<wd:Contact_Information_Data>'''
	...
	output.close()"""
	formerworkerws += '''</wd:Import_Former_Worker_Request>'''
    #formerworkerws += '''<env:Body>'''
    #formerworkerws += '''<env:Envelope>'''
	...
	f.close()
	return formerworkerws, formerworker['former_worker_id']
		#print(r.text)
		...
		for x in range(0,len(personal.index)):
			...
			if personal.iloc[x]['hispanic_or_latino'] != '':	
				if personal.iloc[x]['hispanic_or_latino'].upper() == 'Y':
					output += '''<wd:Hispanic_or_Latino>1</wd:Hispanic_or_Latino>'''
				elif personal.iloc[x]['hispanic_or_latino'].upper() == 'N':
					output += '''<wd:Hispanic_or_Latino>0</wd:Hispanic_or_Latino>'''
	...
	if len(personal) > 0:
		for x in range(0,len(personal.index)):
			if personal.iloc[x]['national_id'] != '':
				...
				output += '<wd:Country_Reference><wd:ID wd:type="ISO_3166-1_Alpha-3_Code">{0}</wd:ID></wd:Country_Reference>'.format(personal.iloc[x]['national_id_iso'])		
				if personal.iloc[x]['issued_date'] != '':
					output += '<wd:Issued_Date>{0}</wd:Issued_Date>'.format(personal.iloc[x]['issued_date'])
				...
				output += '</wd:National_ID_Data>'
				output += '</wd:National_Identifier_Data>'
	return output
...
def parseAddress(address):
	...
	if len(address) > 0:
		for x in range(0,len(address.index)):
			...
			if address.iloc[x]['usage_type'].upper() == 'WORK':
				public = 'true'
			else:
				public = 'false'
			
			output+='''<wd:Usage_Data wd:Public='{0}'><wd:Type_Data wd:Primary='{1}'><wd:Type_Reference><wd:ID wd:type="Communication_Usage_Type_ID">{2}</wd:ID></wd:Type_Reference></wd:Type_Data></wd:Usage_Data>'''.format(public,str(address.iloc[x]['primaryaddress']).lower(),address.iloc[x]['usage_type'].upper())
...
def parsePhone(phone):
	...
	if len(phone) > 0:
		for x in range(0,len(phone.index)):
			...
			output += '''<wd:Usage_Data wd:Public='{0}'><wd:Type_Data wd:Primary='{1}'><wd:Type_Reference><wd:ID wd:type="Communication_Usage_Type_ID">{2}</wd:ID></wd:Type_Reference></wd:Type_Data></wd:Usage_Data>'''.format(public,str(phone.iloc[x]['primaryphone']).lower(),phone.iloc[x]['usage_type'].upper())
